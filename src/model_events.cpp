  ...
  (input.lung_function.fev1_0_ZafarCMAJ_by_sex[1][(*ag).sex]*(*ag).age_baseline
     ...
     (*ag).fev1_slope=_beta_0_prime
       ...
       (*ag).fev1_baseline = (*ag).fev1;
           ...
           output_ex.n_inc_COPD_by_ctime_adi[std::min((int)floor((*ag).time_at_creation+(*ag).local_time), MAX_CTIME-1)][(*ag).adi_quintile-1]+=1;
#endif
}
...
void event_exacerbation_process(agent *ag)
{
     ...
     (*ag).exac_history_severity_second==2 && ((*ag).local_time - (*ag).exac_history_time_second) <1)))
  {
    ...
    output_ex.n_exac_by_ctime_adi[exac_ctime_idx][(*ag).adi_quintile-1]+=1;
    output_ex.n_exac_severity_adi[(*ag).exac_status-1][(*ag).adi_quintile-1]+=1;
    if((*ag).diagnosis==1) output_ex.n_exac_by_ctime_adi_diagnosed[exac_ctime_idx][(*ag).adi_quintile-1]+=1;
